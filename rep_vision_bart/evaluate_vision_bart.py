#!/usr/bin/env python3
"""
Evaluation for Medical Vision-BART - FINAL FIX
Don't replace custom 3D ViT encoder!
"""
...
from train_vision_bart import MedicalReportDataset, build_transforms
from medical_vision_bart import MedicalVisionBART


def evaluate_model(args):
    print("="*80)
    print("EVALUATION: Medical Vision-BART")
    print("="*80)

    # Load model with custom ViT
    print(f"\nLoading model...")
    vision_encoder_path = '/home/muhammedg/fvlm/checkpoints/model.pth'

    full_model = MedicalVisionBART(
        vision_encoder_path=vision_encoder_path,
        decoder_model_name="facebook/bart-base",
        freeze_encoder=False,
    )

    # Load ONLY decoder weights (keep custom encoder!)
    ...
    # Replace ONLY decoder (encoder stays custom 3D ViT)
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader)):
            try:
                ...
                results.append({'generated_report': pred_text, 'ground_truth': ref_text})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default='./checkpoints/vision_bart/final_model')
    parser.add_argument('--csv_file', type=str, default='/home/muhammedg/fvlm/image_first_dataset.csv')
    parser.add_argument('--output_csv', type=str, default='vision_bart_results.csv')
