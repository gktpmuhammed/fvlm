#!/usr/bin/env python3
# Variant: Hard Example Mining — oversample patients the model struggles with
import sys
import os
...
class HardExampleCallback(TrainerCallback):
    """
    After each epoch, recompute sampling weights based on per-sample loss.
    High-loss samples (hard examples) get higher sampling probability.
    """
    def __init__(self, train_dataset, hard_multiplier=3.0, hard_percentile=70):
        self.train_dataset = train_dataset
        self.hard_multiplier = hard_multiplier  # Weight multiplier for hard examples
        self.hard_percentile = hard_percentile  # Top X% by loss are "hard"
        self.sample_losses = {}  # {idx: running_avg_loss}
    
    def update_loss(self, idx, loss_val):
        """Called by the trainer to record per-sample loss."""
        if idx in self.sample_losses:
            # Exponential moving average
            self.sample_losses[idx] = 0.7 * self.sample_losses[idx] + 0.3 * loss_val
        else:
            self.sample_losses[idx] = loss_val
    
    def get_sampler_weights(self):
        """Compute sampling weights based on accumulated losses."""
        n = len(self.train_dataset)
        weights = np.ones(n, dtype=np.float64)
        
        if len(self.sample_losses) < 10:
            return weights  # Not enough data yet
        
        # Collect losses
        losses = np.array([self.sample_losses.get(i, 0.0) for i in range(n)])
        
        # Find threshold for "hard" examples
        threshold = np.percentile(losses[losses > 0], self.hard_percentile)
        
        # Hard examples get higher weight
        hard_mask = losses >= threshold
        weights[hard_mask] = self.hard_multiplier
        
        num_hard = hard_mask.sum()
        print(f"  [HardMining] {num_hard}/{n} hard examples (loss >= {threshold:.4f}), weight={self.hard_multiplier}x")
        
        return weights
    
    def on_epoch_end(self, args, state, control, **kwargs):
        """Rebuild sampler weights after each epoch."""
        weights = self.get_sampler_weights()
        # Store weights on the dataset so the trainer can rebuild the sampler
        self.train_dataset._sampling_weights = weights
        print(f"  [HardMining] Updated sampling weights after epoch {state.epoch:.0f}")
...
def main():
    ...
    # Initialize sampling weights (uniform at start)
    train_ds._sampling_weights = np.ones(len(train_ds), dtype=np.float64)
    hard_cb = HardExampleCallback(train_ds, hard_multiplier=3.0, hard_percentile=70)
    ...
    class MedicalTrainer(Trainer):
        def __init__(self, hard_example_cb=None, **kwargs):
            super().__init__(**kwargs)
            self.hard_example_cb = hard_example_cb
            self._current_sample_idx = 0
        
        def get_train_dataloader(self):
            """Override to use WeightedRandomSampler based on hard example weights."""
            weights = getattr(self.train_dataset, '_sampling_weights', None)
        ...
        def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
            """Override to track per-sample loss for hard example mining."""
            outputs = model(**inputs)
            loss = outputs['loss'] if isinstance(outputs, dict) else outputs[0]
            
            # Track per-sample loss for hard example mining
            if self.hard_example_cb is not None and self.model.training:
                self.hard_example_cb.update_loss(
                    self._current_sample_idx % len(self.train_dataset),
                    loss.item()
                )
    ...
    print(f"[Hard Example Mining] Training with dynamic oversampling (top 30% loss -> 3x weight)")
    trainer.train()
