...
Variant: Perceiver Resampler — multi-layer cross-attention for richer organ embeddings
"""
import sys
import os
import torch
...
class PerceiverBlock(nn.Module):
    """
    A single Perceiver block: Self-Attention among queries, then Cross-Attention to visual features.
    This allows queries to iteratively refine their representation by:
    1. Sharing information between organ queries (self-attn)
    2. Attending to relevant visual features (cross-attn with mask bias)
    """
    def __init__(self, hidden_size=768, num_heads=8, ff_mult=4, dropout=0.1):
        super().__init__()
        # Self-Attention (queries attend to each other)
        self.self_attn = nn.MultiheadAttention(embed_dim=hidden_size, num_heads=num_heads, batch_first=True, dropout=dropout)
        self.norm1 = nn.LayerNorm(hidden_size)
        
        # Cross-Attention (queries attend to visual features)
        self.cross_attn = nn.MultiheadAttention(embed_dim=hidden_size, num_heads=num_heads, batch_first=True, dropout=dropout)
    ...
    def forward(self, queries, kv_feats, attn_bias=None):
        """
        Args:
            queries: (B, N_queries, D) - organ query embeddings
            kv_feats: (B, SeqLen, D) - visual features from ViT
            attn_bias: (B*num_heads, N_queries, SeqLen) - organ mask attention bias
        Returns:
            queries: (B, N_queries, D) - refined queries
        """
        # 1. Self-Attention (queries talk to each other)
        q_normed = self.norm1(queries)
        sa_out, _ = self.self_attn(query=q_normed, key=q_normed, value=q_normed)
        queries = queries + sa_out
        
        # 2. Cross-Attention (queries attend to visual features with mask bias)
        q_normed = self.norm2(queries)
        ca_out, ca_weights = self.cross_attn(query=q_normed, key=kv_feats, value=kv_feats, attn_mask=attn_bias)
        queries = queries + ca_out
        
        # 3. FFN
        queries = queries + self.ffn(self.norm3(queries))
        
        return queries, ca_weights


class PerceiverResampler(nn.Module):
    """
    Multi-layer Perceiver Resampler (inspired by Flamingo / Argus).
    Stacks multiple PerceiverBlocks to iteratively refine organ query embeddings.
    """
    def __init__(self, hidden_size=768, num_heads=8, num_layers=4, ff_mult=4, dropout=0.1):
        super().__init__()
        self.layers = nn.ModuleList([
            PerceiverBlock(hidden_size, num_heads, ff_mult, dropout)
    ...
    def forward(self, queries, kv_feats, attn_bias=None):
        ...
        for layer in self.layers:
            queries, last_weights = layer(queries, kv_feats, attn_bias)
        return self.output_norm(queries), last_weights


class Attentive_ROI_Wrapper(nn.Module):
    """
    ViT -> Perceiver Resampler (multi-layer cross-attention) -> Organ Specific Embeddings
    """
    def __init__(self, vit_model, config, num_organs=12, perceiver_layers=4):
        super().__init__()
        self.vit = vit_model
        ...
        # Perceiver Resampler (replaces single cross-attention layer)
    ...
    def forward(self, pixel_values, organ_masks=None):
        ...
        if organ_masks is not None:
            ...
            num_heads = 8  # Must match perceiver num_heads
            attn_bias = attn_bias.repeat_interleave(num_heads, dim=0)
            
            # --- Perceiver Resampler ---
            queries = self.organ_queries.unsqueeze(0).expand(B, -1, -1)
            
            organ_embeddings, attn_weights = self.perceiver(
                queries=queries,
                kv_feats=image_feats,
                attn_bias=attn_bias
            )
...
class MedicalVLM(nn.Module):
    def __init__(
        ...
        **kwargs 
        ...
        print(f"  Perceiver:      4-layer Resampler (self-attn + cross-attn per layer)")
