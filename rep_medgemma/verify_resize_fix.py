...
from monai.transforms import Compose, LoadImaged, SpatialPadd, Resized, Transposed, EnsureChannelFirstd
...
def process_single_mask(filepath):
    try:
        # TEST THE NEW RESIZE PIPELINE
        # Construct list ensuring no None values
        t_list = [
            LoadImaged(keys=['mask'], reader='ITKReader'), 
            EnsureChannelFirstd(keys=['mask']),
            Transposed(keys=['mask'], indices=(0, 3, 2, 1)),
            # ScaleIntensityRanged skipped (mask doesn't need intensity scaling like image)
            SpatialPadd(keys=['mask'], spatial_size=(112, 256, 352), mode='constant', constant_values=0),
            Resized(keys=['mask'], spatial_size=(112, 256, 352), mode='nearest'), # Nearest for mask
        ]
...
def analyze_dataset(mask_folder, output_dir, file_extension="*.nii.gz", num_workers=None, sample_size=100):
    ...
    print(f"Starting Resize Verification with {num_workers} workers...")
    ...
    print("\n=== ORGAN FREQUENCY (WITH RESIZE FIX) ===")
    print(df.to_string())
    
    df.to_csv(os.path.join(output_dir, "resize_frequency_summary.csv"), index=False)
...
if __name__ == "__main__":
    ...
    analyze_dataset(args.mask_folder, './mask_analysis_resized', sample_size=args.sample_size)
