...
Variant: Multi-Scale ViT FPN — fuses intermediate ViT block features (3, 6, 9, final)
"""
import sys
import os
import torch
...
class MultiScaleFusion(nn.Module):
    """
    Fuses features from multiple ViT blocks (3, 6, 9, final).
    Each scale gets its own LayerNorm (to handle different feature distributions),
    weighted by learned importance scores, then concatenated and projected back.
    """
    def __init__(self, hidden_size=768, num_scales=4):
        super().__init__()
        self.num_scales = num_scales
        self.scale_norms = nn.ModuleList([
            nn.LayerNorm(hidden_size) for _ in range(num_scales)
        ])
        # Learned per-scale importance weights (initialized equal)
        self.scale_weights = nn.Parameter(torch.ones(num_scales))
        # Fuse concatenated multi-scale features back to hidden_size
        self.fusion_proj = nn.Linear(hidden_size * num_scales, hidden_size)
        nn.init.normal_(self.fusion_proj.weight, std=0.01)
        nn.init.zeros_(self.fusion_proj.bias)

    def forward(self, multi_scale_feats):
        """
        Args:
            multi_scale_feats: list of num_scales tensors, each (B, SeqLen, D)
        Returns:
            fused: (B, SeqLen, D)
        """
        weights = F.softmax(self.scale_weights, dim=0)
        normed = [
            self.scale_norms[i](feat) * weights[i]
            for i, feat in enumerate(multi_scale_feats)
        ]
        concat = torch.cat(normed, dim=-1)    # (B, SeqLen, D*4)
        fused = self.fusion_proj(concat)       # (B, SeqLen, D)
        return fused


class Attentive_ROI_Wrapper(nn.Module):
    """
    ViT -> Multi-Scale FPN -> Masked Cross-Attention -> Organ Specific Embeddings
    """
    def __init__(self, vit_model, config, num_organs=12):
        ...
        self.multi_scale_fusion = MultiScaleFusion(hidden_size=self.hidden_size, num_scales=4)
    ...
    def forward(self, pixel_values, organ_masks=None):
        ...
        # ViT returns (final_feats, [block3, block6, block9, final])
        if isinstance(outputs, tuple) and len(outputs) == 2:
            final_feats, multi_scale_feats = outputs
            if isinstance(multi_scale_feats, list) and len(multi_scale_feats) >= 4:
                # Fuse features from blocks 3, 6, 9, and final output
                image_feats = self.multi_scale_fusion(multi_scale_feats[:4])
            else:
                image_feats = final_feats
        ...
        if organ_masks is not None:
            ...
            # F.interpolate(mode='area') averages, which dilutes small organs like pancreas/gallbladder
...
class MedicalVLM(nn.Module):
    def __init__(
        ...
        **kwargs 
        ...
        print(f"  Multi-Scale FPN: Fusing ViT blocks 3, 6, 9, final")
    ...
    def save_pretrained(self, output_dir):
        ...
        print(f"  [FPN] MultiScaleFusion saved inside vision_encoder.bin")