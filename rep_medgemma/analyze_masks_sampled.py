...
# Training Organs: ['lung', 'heart', 'esophagus', 'liver', 'gallbladder', 'stomach', 'pancreas', 'spleen', 'kidney', 'aorta', 'trachea', 'rib']
MERGED_GROUPS = {
    'lung': [10, 11, 12, 13, 14],
    'heart': [51, 61],
    'esophagus': [15],
...
def process_single_mask(filepath):
    try:
        ...
        return (pid, found_merged)
    except Exception as e:
        return (None, str(e))

def analyze_dataset(mask_folder, output_dir, file_extension="*.nii.gz", num_workers=None, sample_size=500):
    ...
    for pid, merged_list in results:
    ...
    # Save
    df_sum = pd.DataFrame.from_dict(global_merged_counts, orient='index', columns=['count']).reset_index()
    df_sum.rename(columns={'index': 'organ'}, inplace=True)
    df_sum['percentage'] = (df_sum['count'] / len(merged_records)) * 100
    df_sum = df_sum.sort_values(by='count', ascending=False)
    
    print("\n=== ORGAN FREQUENCY (SAMPLED) ===")
    print(df_sum.to_string())
    
    df_sum.to_csv(os.path.join(output_dir, "sampled_frequency_summary.csv"), index=False)
    print(f"\nAnalysis Complete. Results in {output_dir}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--mask_folder', type=str, required=True)
    parser.add_argument('--output_dir', type=str, default='./mask_analysis_sampled')
    
    args = parser.parse_args()
    analyze_dataset(args.mask_folder, args.output_dir, sample_size=500)
