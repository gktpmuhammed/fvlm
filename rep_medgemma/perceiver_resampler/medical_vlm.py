...
class PerceiverBlock(nn.Module):
    ...
    def forward(self, queries, kv_feats, attn_bias=None):
        ...
        ca_out, _ = self.cross_attn(query=q_normed, key=kv_feats, value=kv_feats, attn_mask=attn_bias)
...
class PerceiverResampler(nn.Module):
    ...
    def forward(self, queries, kv_feats, attn_bias=None):
        for layer in self.layers:
            queries = layer(queries, kv_feats, attn_bias)
...
class Attentive_ROI_Wrapper(nn.Module):
    ...
    def forward(self, pixel_values, organ_masks=None):
        ...
        if organ_masks is not None:
            ...
            organ_embeddings = self.perceiver(
                queries=queries,
