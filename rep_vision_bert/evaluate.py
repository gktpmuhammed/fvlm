...
def evaluate(args):
    ...
    with torch.no_grad():
        for batch in tqdm(dl):
            ...
            # Pad to same length
            max_prompt_len = max(len(ids) for ids in prompt_id_list)
            ...
            decoder_input_ids = torch.tensor(padded_ids, dtype=torch.long, device=device)
            decoder_attention_mask = torch.tensor(padded_mask, dtype=torch.long, device=device)
